                ...
                return (
                  <div key={stage}>
                    ...
                    {inStage.map(c => {
                      ...
                      const app = c.applications.find(a => a.job_id === selectedJob.id)
                      return (
                        <div
                          key={c.id}
                          onClick={() => {
                          ...
                          style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', cursor: 'pointer', background: isSelected ? 'var(--accent-bg)' : 'transparent', transition: 'background 0.1s', outline: selectMode && selected.has(c.id) ? '2px solid var(--accent)' : 'none', outlineOffset: -2 }}
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            {selectMode && (
                              <input type="checkbox" checked={selected.has(c.id)} onChange={() => toggleSelect(c.id)} onClick={e => e.stopPropagation()} />
                            )}
                            <div className="avatar" style={{ width: 30, height: 30, fontSize: 11, background: av.bg, color: av.color }}>{initials(c.fname, c.lname)}</div>
                          ...
                          {/* Questionnaire + quick reject */}
                          {app && ((app.work_authorized !== null && app.work_authorized !== undefined) || app.salary_expectations || stage !== 'Rejected') && (
                            <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4, marginTop: 6, paddingLeft: 40 }} onClick={e => e.stopPropagation()}>
                              {app.work_authorized !== null && app.work_authorized !== undefined && (
                                <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 20, fontWeight: 600, background: app.work_authorized ? '#DCFCE7' : '#FEE2E2', color: app.work_authorized ? '#166534' : '#991B1B' }}>
                                  {app.work_authorized ? '✓ Work auth' : '✗ No work auth'}
                                </span>
                              )}
                              ...
                              {stage !== 'Rejected' && stage !== 'Hired' && (
                                <button
                                  style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 8px', borderRadius: 6, background: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA', cursor: 'pointer', fontWeight: 600 }}
                                  onClick={e => { e.stopPropagation(); handleMoveStage(app.id, 'Rejected') }}
                                >
                                  Reject
                                </button>
                              )}
